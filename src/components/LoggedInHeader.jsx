      ...
      {/* sidebar will only show when bars in header are clicked on in phone view */}
        ...
        <div className='flex absolute pin-t pin-r invisible lg:visible mx-8 mt-6 font-josefin text-xs'>
          <button 
            ...
            className={this.state.projectActive ? 'mx-4 hover:text-palette-blue text-palette-blue focus:outline-none' : 'mx-4 text-white hover:text-palette-blue focus:outline-none'}
          >PROJECTS</button>
          <button 
            onClick={ this.teamClick } 
            className={this.state.teamActive ? 'mx-4 hover:text-palette-blue text-palette-blue focus:outline-none' : 'mx-4 text-white hover:text-palette-blue focus:outline-none'}>TEAMS
          </button>
          <button 
            onClick={ () => this.logout() }
